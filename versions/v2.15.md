---
layout: page
title: 'Roadmap v2.15 Documentation'
permalink: /versions/v2.15/
sortable_version: 215
---
## Table of contents
<!--ts-->
...
Roadmap was first developed in 2019 by Caleb Braun, Lingzhi Jin, and Josh Miller. Version 2.15 was developed in 2025 by Gabe Hillman Alvarez and Arijit Sen.
...
Tailpipe emission factors are sourced from a combination of emission factor models, remote sensing campaigns, ICCT analyses, and other technical studies. Emission factors for vehicles certified to European-equivalent standards are based on real-world measurements from The Real Urban Emissions Initiative ([TRUE](https://www.trueinitiative.org/)), the European Environment Agency’s European Monitoring and Evaluation Programme (EMEP), the Handbook Emission Factors for Road Transport (HBEFA), and ICCT adjustments (e.g., for deterioration). Emission factors for vehicles certified to US- and China-equivalent standards are sourced from EPA’s MOtor Vehicle Emission Simulator (MOVES) with ICCT adjustments (e.g., based on real-world measurements) and ICCT's China model, respectively. Currently the Roadmap model does not estimate non-exhaust emissions such as brake and tire wear.
...
For the United States and Canada, vehicle emission factors are based on EPA's MOVES4 model. MOVES estimates diesel, gasoline, and natural gas emission factors for vehicles following the US standards. Rather than binning emission factors based on EPA’s emission standards (e.g. US 2007 and US 2010), Roadmap preserves the level of detail in MOVES4, incorporating emission factors for all model years from 1990 to 2025; these emission factors were calculated using MOVES4 outputs of emissions and energy consumption for available vehicle and fuel combinations. For Mexico, which allows new vehicles to be certified to US or EU standards, new heavy-duty vehicles are assumed to be partially sourced from the USA and thus are assigned a weighted average emission factor based on US and EU standards. China-specific emission factors are sourced from the ICCT’s China model, which incorporates data from the Vehicle Emission Control Center and Tsinghua University.

Further adjustments were made to the PM and NOx emission factors certified to US regulatory pathways based on real-world performance data, including remote sensing measurements, recent PEMS testing, and planned updates to MOVES. For NOx, the MOVES4-based emission factors for the US were adjusted to align with real-world emissions data from TRUE campaigns in the US. Roadmap also supports additional speciation of PM<sub>2.5</sub> for all regions into components BC, OC, non-carbon organic matter, NO<sub>3</sub>, NH<sub>4</sub>, SO<sub>4</sub>, elemental metals, and unspeciated PM, based on Table C-1 and Table C-3 of [MOVES 2014](https://cfpub.epa.gov/si/si_public_file_download.cfm?p_download_id=525697&Lab=OTAQ)a. For the US only, no PM speciation is required, because MOVES4 provides detailed emission factors for the components directly.
...
Roadmap can model emission deterioration by using age-specific multipliers for emission factors. Except for NO<sub>x</sub> emission factor multipliers in USA HDVs (which are based on ICCT analysis), emission factor inputs represent lifetime average emission rates; in these cases, to avoid double-counting, emission deterioration is set to 1 for all ages. In cases where zero-mile emission rates are input and deterioration is modelled separately, the multiplier will increase from 1 as the vehicle ages, meaning that the vehicle will produce more emissions than when it was brand new.
...
<button name='download' onclick="location.href='../Roadmap v2.14 Model Documentation.pdf'">Download as PDF</button>

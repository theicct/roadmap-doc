---
layout: page
title: 'Roadmap v1.7 Documentation'
permalink: /versions/v1.7/
---
## Table of contents
<!--ts-->
  ...
  * [Plug-in Hybrid Electric Vehicles (PHEV)](#plug-in-hybrid-electric-vehicles-phev)
...
<!-- Added by: erikpronk, at: Wed Aug  4 10:23:08 PDT 2021 -->
...
-   EU sales totals are derived from [ACEA](https://www.acea.auto/reliable-data-statistics/) data;
...
Stock<sub>*C**Y*, *M**Y*</sub>= Stock<sub>*C**Y* − 1, *M**Y*</sub> × Survival<sub>*C*, *V*</sub>(*C**Y*−*M**Y*)

In the case of age zero vehicles (CY = MY), sales are used instead of the previous year’s stock:

Stock<sub>*C**Y*, *M**Y*</sub>= Sales<sub>MY</sub> × Survival<sub>*C*, *V*</sub>(0)
...
$$\\text{SO}\_{2}(g) = \\ \\text{FC}(g)\\  \\times \\ \\lbrack S\\rbrack\\left( \\text{ppm} \\right) \\times \\ \\frac{\\text{MW}\_{\\text{SO}\_{2}}\\ }{\\text{MW}\_{S}}\\  \\times \\ f\\text{SO}\_{2}\\  \\times \\ \\left( \\frac{10^{6}}{\\text{ppm}} \\right)$$

Where:

-   FC(*g*)  = fuel consumption (grams)

-   $$*S*$$(ppm)  = fuel-sulfur level (ppm)

-   $\\frac{\\text{MW}\_{\\text{SO}\_{2}}\\ }{\\text{MW}\_{S}}$ = the ratio of molecular weight of sulfur dioxide to sulfur = 2.0

-   *f*SO<sub>2</sub> = Fraction of fuel sulfur that is converted to gaseous SO<sub>2</sub> emissions (1 - fraction converted to SO<sub>4</sub>).

This equation simplifies to:

SO<sub>2</sub>(*g*) = FC(*g*) × SO<sub>2</sub> EF (*g*/ppm) × *S*(ppm)

where SO<sub>2</sub>EF is the emission factor specific to HDVs and LDVs by emission control technology.

SO<sub>4</sub> emissions are calculated:

$${\\text{SO}4}\_{x} = \\text{NonECPMB}\\  \\times \\ S\_{B}\\  \\times \\ \\left\\lbrack 1 + \\ F\_{B}\\  \\times \\ \\left( \\frac{x}{x\_{B}} - 1 \\right) \\right\\rbrack$$

Where:

-   NonECPMB = the reference non-elemental carbon PM<sub>2.5</sub> emission rate

-   *S*<sub>*B*</sub> = the sulfate reference fraction

-   *x* = the user-supplied fuel sulfur level

-   *x*<sub>*B*</sub> = the reference fuel sulfur level

-   *F*<sub>*B*</sub> = the percentage of sulfate originating from the fuel sulfur in the reference case

-   *S**O*4<sub>*x*</sub> = sulfate emissions at the user-supplied fuel sulfur level

The *S*<sub>*B*</sub>, *F*<sub>*B*</sub>, and *x*<sub>*B*</sub>, parameters vary by vehicle type, model year group, and emission process.
...
Base year global inputs for the GHG intensity of electricity generation are sourced from [IPCC 2011](https://www.ipcc.ch/site/assets/uploads/2018/02/ipcc_wg3_ar5_annex-iii.pdf). These are combined with the projected share of electricity generation by type from IEA’s World Energy Outlook (WEO) 2020 STEPS scenario for the USA, China, India, Japan, and the EU to obtain a final aggregate CO<sub>2</sub> emission value for generation. This also allows for other scenarios of generation shares to be specified by the user. The emission intensity for electricity generation in other regions is based on the same scenario, but instead uses only the aggregated value.
...
<button name='download' onclick="location.href='../Roadmap v1.7 Model Documentation.pdf'">Download as PDF</button>

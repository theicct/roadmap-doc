...
<!-- Added by: caleb, at: Mon Mar  1 16:23:35 PST 2021 -->
...
![](/assets/high_ems_age_effects.svg)
...
where \\(\text{SO}_{2}\text{\ EF}\\) is the emission factor specific to
HDVs and LDVs by emission control technology.
...
Where:
  ...
  - \\(F_{B}\\) = the percentage of sulfate originating from the fuel
    sulfur in the reference case

  - \\({SO4}_{x}\\) = sulfate emissions at the user-supplied fuel sulfur
    level

The \\(S_{B}\\), \\(F_{B}\\), and \\(x_{B}\\), parameters vary by vehicle
...
<button name='download' onclick='/Roadmap v1.5 Model Documentation.pdf'>Download as PDF</button>
